...
class CustomOptimizer(object):
    # DM: modification to Torch's base Optimizer to save param groups for 3 models instead of one
    ...
    def __init__(self, params_x, params_y, params_v, defaults):
        ...
        # params X
        if isinstance(params_x, torch.Tensor):
            raise TypeError("params argument given to the optimizer should be "
                            "an iterable of Tensors or dicts, but got " +
                            torch.typename(params_x))

        param_groups_x = list(params_x)
        if len(param_groups_x) == 0:
            raise ValueError("optimizer got an empty parameter list")
        if not isinstance(param_groups_x[0], dict):
            param_groups_x = [{'params': param_groups_x}]

        for param_group in param_groups_x:
            self.add_param_group(param_group, self.param_groups_x)

        # params Y
        if isinstance(params_y, torch.Tensor):
            raise TypeError("params argument given to the optimizer should be "
                            "an iterable of Tensors or dicts, but got " +
        ...
        if isinstance(params_v, torch.Tensor):
            raise TypeError("params argument given to the optimizer should be "
                            "an iterable of Tensors or dicts, but got " +
                            torch.typename(params_v))

        param_groups_v = list(params_v)
        if len(param_groups_v) == 0:
            raise ValueError("optimizer got an empty parameter list")
        if not isinstance(param_groups_v[0], dict):
            param_groups_v = [{'params': param_groups_v}]

        for param_group in param_groups_v:
            self.add_param_group(param_group, self.param_groups_v)
    ...
    def add_param_group(self, param_group, param_groups):

...
from utils import save_experiment, load_results, save_lr_sweep, load_sweep_results, get_sweep_filename, get_folder_name


def sweep_step_sizes(config, expt, early_stop=False, save_expts=False, save_sweep=True, root=None):
    arr_steps = []
    arr_accuracy = []
    arr_test_loss = []
    arr_train_loss = []
    ...
    if save_sweep:
        if root is None:
            save_lr_sweep(config, arr_steps, arr_accuracy, arr_test_loss, arr_train_loss)
        else:
            save_lr_sweep(config, arr_steps, arr_accuracy, arr_test_loss, arr_train_loss, root=root)
